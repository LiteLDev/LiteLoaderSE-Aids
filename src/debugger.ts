...
export function runTerminal() {
  var bdsDir = String(
    vscode.workspace
      .getConfiguration()
      .get("LLScriptHelper.bds-LLScriptHelperDir")
  );
  var bds = String(
    vscode.workspace.getConfiguration().get("LLScriptHelper.bds-TaskCmd")
  );
  var args: string[] = vscode.workspace
    .getConfiguration("")
    .get("LLScriptHelper.bds-TaskCmd-Args")!;
  if (bdsDir === "null" || bdsDir === null) {
    ...
    if (terminal === undefined && os.platform() === "win32") {
      ...
      vscode.workspace
        .getConfiguration()
        .update("LLScriptHelper.isrunning", true);
    } else {
      terminal = vscode.window.createTerminal({
        name: "LiteLoaderScript Dev",
        shellPath: bds,
...
export function loadPlugins(fileUri: vscode.Uri) {
  if (terminal === undefined) {
    vscode.workspace
      .getConfiguration()
      .update("LLScriptHelper.isrunning", false);
    vscode.window
      .showInformationMessage("LiteLoader控制台未启动", "启动")
      .then(function (t) {
        if (t === "启动") {
          runTerminal();
        }
...
export function reloadPlugins(fileUri: vscode.Uri) {
  if (terminal === undefined) {
    vscode.workspace
      ...
      .update("LLScriptHelper.isrunning", false);
    vscode.window
      .showInformationMessage("BDS控制台未启动", "启动")
      .then(function (t) {
        if (t === "启动") {
          runTerminal();
        }
...
function selectedDir() {
  vscode.window
    .showInformationMessage("未配置带有LiteLoader环境的BDS目录", "选择目录")
    .then(function (t) {
      if (t === "选择目录") {
        vscode.window
          .showOpenDialog({
          ...
          .then(function (uri) {
            if (uri !== undefined) {
              ...
              vscode.workspace
                .getConfiguration("")
                .update("LLScriptHelper.bds-LLScriptHelperDir", uris);
...
function selectedTask(uris: string) {
  ...
  if (os.platform() !== "win32") {
    ...
    if (fileList.indexOf("bedrock_server.exe") != -1) {
      vscode.workspace
        .getConfiguration("")
        .update("LLScriptHelper.bds-TaskCmd", "bedrock_server.exe");
      vscode.workspace
        .getConfiguration("")
        .update("LLScriptHelper.bds-TaskCmd-Args", ["| pause"]);
      vscode.window.showInformationMessage(
        "启动命令选择成功：" + "bedrock_server.exe"
      );
    } else if (fileList.indexOf("bedrock_server_mod.exe") != -1) {
      vscode.workspace
        .getConfiguration("")
        .update("LLScriptHelper.bds-TaskCmd", "bedrock_server_mod.exe");
      vscode.workspace
        .getConfiguration("")
        .update("LLScriptHelper.bds-TaskCmd-Args", ["| pause"]);
      vscode.window.showInformationMessage(
        "启动命令选择成功：" + "bedrock_server_mod.exe"
      );
    } else {
      vscode.window
        .showWarningMessage("没有找到BDS主程序", "手动输入")
        .then(async function (t) {
          if (t === "手动输入") {
            let task = await vscode.window.showInputBox({
              placeHolder: "请输入您的启动命令",
            });
            if (task) {
              ...
              vscode.workspace
                .getConfiguration("")
                .update("LLScriptHelper.bds-TaskCmd", uriself);
              vscode.workspace
                .getConfiguration("")
                .update("LLScriptHelper.bds-TaskCmd-Args", []);
    ...
    vscode.workspace
      .getConfiguration("")
      .update("LLScriptHelper.bds-TaskCmd", "bedrock_server");
    vscode.workspace
      .getConfiguration("")
      .update("LLScriptHelper.bds-TaskCmd-Args", []);
    vscode.window.showWarningMessage(
      "无法推导BDS启动命令,请打开settings.json手动设置",
      "我知道了"
    );
