...
export class LibraryConfig {
  public run(arrs: any) {
    if (
      vscode.workspace.getConfiguration().get("LLScriptHelper.LibraryPath") !==
        null &&
      vscode.workspace.getConfiguration().get("LLScriptHelper.LibraryPath") !==
        ""
    ) {
      const ver = vscode.workspace
        .getConfiguration()
        .get("LLScriptHelper.version");
      vscode.window
        .showInformationMessage(arrs.notice, "更新")
        .then(function (t) {
          if (t === "更新") {
            vscode.window.showInformationMessage(
              "开始更新Library：" + ver + " -> " + arrs.version
            );
      ...
      vscode.window
        ...
        .then(function (t) {
          if (t === "使用默认") {
            const libary = vscode.extensions.getExtension(
              "moxicat.LLScriptHelper"
            );
            const path = libary?.extensionPath + "/Library";
            vscode.workspace
              .getConfiguration()
              .update("LLScriptHelper.LibraryPath", path, true)
              .then(function () {
                vscode.window.showInformationMessage(
                  "开始首次配置Library：" + path
                );
            ...
            vscode.window
              ...
              .then(function (uri) {
                if (uri !== undefined) {
                  ...
                  vscode.workspace
                    .getConfiguration()
                    .update("LLScriptHelper.LibraryPath", uris, true)
                    .then(function () {
                      vscode.window.showInformationMessage(
                        "开始首次配置Library：" + uris
                      );
                      new LibraryConfig().downloadFile(
                        arrs.download,
                        uris,
                        arrs
                      );
          ...
          zip.extract(null, dirPath, (err: any, count: any) => {
            if (err) {
              vscode.window.showErrorMessage(
                "LLScriptHelper-Libary下载失败，请检查网络"
              );
            } else {
              fs.unlinkSync(filepath);
              vscode.window.showInformationMessage(
                "LLScriptHelper-Library 版本：" + arrs.version
              );
              vscode.workspace
                .getConfiguration()
                .update("LLScriptHelper.version", arrs.version, true);
              vscode.workspace
                .getConfiguration()
                .update("Lua.workspace.library", [dirPath + "/Lua"], true);
              vscode.window.showInformationMessage(
                "LLScriptHelper-Library 已更新"
              );
            }
            zip.close();
          });
